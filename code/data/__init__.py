...
def create_dataset(option_ds: dict) -> Dataset:
    ...
    if name == 'dataset_cl_2folder':
        from data.dataset_CL_2folder import Dataset_CL_2folder as D
    else:
    ...
    logger.info(
        'Dataset [{:s} - {:s}] is created.'.format(
            dataset.__class__.__name__, option_ds['name']))
    return dataset

def create_dataloader(dataset: Dataset, option_ds: dict, gpu_ids=None) -> DataLoader:
    """
    Create Dataloader.
