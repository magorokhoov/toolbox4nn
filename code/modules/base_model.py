...
class BaseModel:
    def __init__(self, option:dict):
        ...
        for network_name in option_networks:
            ...
            self.optimizers[network_name] = optimizers.get_optimizer(self.networks[network_name].parameters(), option_optimizer)
            ...
            self.schedulers[network_name] = schedulers.get_scheduler(self.optimizers[network_name], option_scheduler) # torch.optim.lr_scheduler.ExponentialLR(self.optimizers[network_name], gamma=0.9)
    ...
    def train_step(self) -> None:
        ...
        self.logger.warn('Please do not use base_model. Use concrete pipeline instand')
        raise RuntimeError('Please do not use base_model. Use concrete pipeline instand')
            ...
            for epoch in range(1, n_epochs+1):
                for self.sample in self.dataloader:
                    ...
                    if i % self.print_freq == 0:
                        ...
                        self.logger.info(f'<epoch: {epoch}, iter: {i}, {time_string:s}> | {info_str}')
    ...
    def logger_info_networks_params(self) -> None:
        ...
        for network_name in self.networks:
            self.logger.info(f'{network_name}: {self.networks[network_name].get_num_parameters():,}')
    ...
    def save_models(self, iteration=0, is_last:bool=False) -> None:
        ...
        for network_name in self.networks:
            self.network_path = os.path.join(self.models_dir_path, f'{prefix}{network_name}.pth')
            torch.save(self.networks[network_name].state_dict(), self.network_path)
    ...
    def load_models(self, option_weights:dict) -> None:
        for network_name in self.networks:
            ...
            if network_path is not None:
                self.networks[network_name].load_state_dict(torch.load(network_path))